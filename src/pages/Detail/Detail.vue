...
// 商品详细信息
const detailInfo = ref<DetailItem>()
// 商品图片长度
...
// 图片展示轮播图下标变化
const onChangeImg = (e) => {
  // const showImgIndex = computed(() => {
  //   return e.detail.current + 1
  // })
  showImgIndex.value = e.detail.current + 1
...
<template>
  ...
  <scroll-view scroll-y class="viewport">
    ...
    <view class="goods">
      <!-- 商品主图 -->
      <view class="preview">
        <swiper circular @change="onChangeImg">
          <swiper-item v-for="(value, index) in detailInfo?.mainPictures" :key="index">
            <image mode="aspectFill" :src="value" />
          </swiper-item>
        </swiper>
        <view class="indicator">
          <text class="current">{{ showImgIndex }}</text>
          <text class="split">/</text>
          <text class="total">{{ imgLength }}</text>
        </view>
      </view>
      ...
      <view class="meta">
        <view class="price">
          ...
          <text class="number">{{ detailInfo?.price }}</text>
        </view>
        <view class="name ellipsis">{{ detailInfo?.name }} </view>
      ...
      <view class="action">
        ...
        <view @click="openPopup('service')" class="item arrow">
    ...
    <view class="detail panel">
      ...
      <view class="content">
        <view class="properties">
          <!-- 属性详情 -->
          <view class="item" v-for="(value, index) in detailInfo?.details.properties" :key="index">
            <text class="label">{{ value.name }}</text>
            <text class="value">{{ value.value }}</text>
          </view>
        </view>
        <!-- 图片详情 -->
        <image
          v-for="(img, index) in detailInfo?.details.pictures"
          :key="index"
          mode="widthFix"
          :src="img"
        ></image>
    ...
    <view class="similar panel">
      ...
      <view class="content">
        <XtxGoodsItem
          v-for="goods in detailInfo?.similarProducts"
          :key="goods.id"
          :goods="goods"
        ></XtxGoodsItem>
      </view>
